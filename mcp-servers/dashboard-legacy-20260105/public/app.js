...
class MissionControl {
  ...
  bindElements() {
    ...
    this.elements.addRepoForm = document.getElementById('addRepoForm');
    this.elements.repoName = document.getElementById('repoName');
    this.elements.repoPath = document.getElementById('repoPath');
  ...
  bindEvents() {
    ...
    // Repo Browser
    this.elements.addRepoBtn?.addEventListener('click', () => this.openRepoForm());
    this.elements.addRepoClose?.addEventListener('click', () => this.closeRepoForm());
    this.elements.addRepoCancel?.addEventListener('click', () => this.closeRepoForm());
    this.elements.addRepoForm?.addEventListener('submit', (e) => {
      e.preventDefault();
      this.submitAddRepo();
    });
    this.elements.browsePathBtn?.addEventListener('click', () => this.browseDirectory());
    this.elements.addRepoOverlay?.addEventListener('click', (e) => {
      if (e.target === this.elements.addRepoOverlay) this.closeRepoForm();
    });
  ...
  openRepoForm() {
    ...
    // Clear form
    if (this.elements.repoName) this.elements.repoName.value = '';
    if (this.elements.repoPath) this.elements.repoPath.value = '';
    // Remove readonly from path input
    if (this.elements.repoPath) this.elements.repoPath.removeAttribute('readonly');
    // Focus on name input
    setTimeout(() => this.elements.repoName?.focus(), 100);
  }

  async browseDirectory() {
    console.log('browseDirectory called');
    // Open a server-side directory browser modal
    // This uses the existing /api/browse endpoint which works properly
    try {
      await this.openDirectoryBrowser();
    } catch (error) {
      console.error('Error opening directory browser:', error);
      this.showToast('error', 'Error', 'Failed to open directory browser');
    }
  }

  async openDirectoryBrowser() {
    console.log('openDirectoryBrowser called');
    // Create a directory browser modal
    const currentPath = this.elements.repoPath?.value || '/Users/ctavolazzi/Code';
    console.log('Current path:', currentPath);

    // Create browser modal HTML
    const browserHTML = `
      <div class="directory-browser">
        <div class="browser-toolbar">
          <button class="browser-btn" id="browserUp" title="Go Up">↑</button>
          <input type="text" class="browser-path-input" id="browserPathInput" value="${this.escapeHtml(currentPath)}" placeholder="/path/to/directory">
          <button class="browser-btn" id="browserGo" title="Go to Path">Go</button>
          <button class="browser-btn" id="browserRefresh" title="Refresh">⟳</button>
        </div>
        <div class="browser-search-bar">
          <input type="text" class="browser-search" id="browserSearch" placeholder="🔍 Search directories...">
          <button class="browser-clear-search" id="browserClearSearch" title="Clear search">×</button>
        </div>
        <div class="browser-breadcrumbs" id="browserBreadcrumbs"></div>
        <div class="browser-list" id="browserList">
          <div class="browser-loading">Loading...</div>
        </div>
        <div class="browser-footer">
          <span class="browser-hint">💡 Single-click to navigate, double-click to select</span>
        </div>
      </div>
    `;

    // Find the specific modal body for add repo - must be inside addRepoOverlay
    const addRepoOverlay = document.getElementById('addRepoOverlay');
    const modalBody = addRepoOverlay?.querySelector('.modal-body');

    // Replace form with browser
    if (modalBody) {
      console.log('Replacing modal body with browser', {
        modalBody: modalBody,
        overlay: addRepoOverlay,
        currentHTML: modalBody.innerHTML.substring(0, 100)
      });

      // Replace synchronously
      modalBody.innerHTML = browserHTML;

      console.log('After replacement:', {
        newHTML: modalBody.innerHTML.substring(0, 200),
        hasBrowserList: !!document.getElementById('browserList')
      });

      // Bind browser events - must happen AFTER innerHTML is set
      this.browserPath = currentPath;
      this.browserParent = null;
      this.browserItems = [];
      ...
      const clearSearchBtn = document.getElementById('browserClearSearch');
      const browserList = document.getElementById('browserList');

      upBtn?.addEventListener('click', () => this.browseParent());
      goBtn?.addEventListener('click', () => {
        const path = pathInput?.value.trim();
        if (path) this.browsePath(path);
      });
      refreshBtn?.addEventListener('click', () => this.browsePath(this.browserPath));
      ...
      searchInput?.addEventListener('input', (e) => {
        this.browserSearchTerm = e.target.value;
      ...
      // Initial search clear button state
      this.updateSearchClearButton();
      ...
      console.error('Modal body not found!', {
        modalBody: modalBody,
        addRepoForm: this.elements.addRepoForm,
        parentElement: this.elements.addRepoForm?.parentElement
      });
      this.showToast('error', 'Error', 'Could not find modal body');
    }
  }
  ...
  async browsePath(dirPath) {
    ...
    // Update breadcrumbs
    this.updateBreadcrumbs(dirPath);

    // Show loading state
    this.updateBrowserLoadingState(true);
    browserList.innerHTML = '<div class="browser-loading"><span class="browser-spinner">⟳</span> Scanning directories...</div>';
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || `HTTP ${res.status}: Failed to browse directory`);
      }

      const data = await res.json();

      const upBtn = document.getElementById('browserUp');
      if (upBtn) upBtn.disabled = !data.canGoUp;
  ...
  updateBrowserLoadingState(isLoading) {
    const browserList = document.getElementById('browserList');
    const allItems = browserList?.querySelectorAll('.browser-item');
  ...
  applySearchFilter() {
    ...
    const filteredItems = this.filterDirectories(this.browserSearchTerm, this.browserItems);

    if (filteredItems.length === 0) {
      if (this.browserSearchTerm) {
        browserList.innerHTML = `<div class="browser-loading">No directories match "${this.escapeHtml(this.browserSearchTerm)}"</div>`;
      } else {
        browserList.innerHTML = '<div class="browser-loading">No folders found</div>';
      }
      return;
    }

    browserList.innerHTML = filteredItems.map(item => `
      <div class="browser-item ${item.hasWorkEfforts ? 'has-work-efforts' : ''}"
        ...
        <div class="browser-item-content">
          ...
          ${item.hasWorkEfforts ? `
            <div class="browser-item-meta">
              <span class="browser-item-badge">Has Work Efforts</span>
    ...
    // Bind click events - single-click navigates, double-click selects
  ...
  updateSearchClearButton() {
    const clearSearchBtn = document.getElementById('browserClearSearch');
    if (clearSearchBtn) {
      clearSearchBtn.style.display = this.browserSearchTerm ? 'flex' : 'none';
    }
  }

  updateBreadcrumbs(currentPath) {
    const breadcrumbsContainer = document.getElementById('browserBreadcrumbs');
    if (!breadcrumbsContainer) return;

    const breadcrumbs = this.renderBreadcrumbs(currentPath);
    breadcrumbsContainer.innerHTML = breadcrumbs;

    // Bind breadcrumb clicks
    breadcrumbsContainer.querySelectorAll('.breadcrumb-segment').forEach(segment => {
  ...
  renderBreadcrumbs(currentPath) {
    ...
    if (parts.length === 0) {
      return '<span class="breadcrumb-segment active">/</span>';
    }

    const segments = [];
    let accumulatedPath = isAbsolute ? '' : '';

    parts.forEach((part, index) => {
      accumulatedPath += (accumulatedPath ? '/' : (isAbsolute ? '/' : '')) + part;
      segments.push({
        name: part,
  ...
  showBrowserError(error) {
    ...
    browserList.innerHTML = `
      <div class="browser-error">
        ...
        <div class="browser-error-message">${this.escapeHtml(errorMessage)}</div>
        <button class="browser-retry-btn" id="browserRetryBtn">Retry</button>
      </div>
    `;

    // Bind retry button
    const retryBtn = document.getElementById('browserRetryBtn');
    retryBtn?.addEventListener('click', () => {
      this.browsePath(this.browserPath);
    });

    // Show toast notification
    this.showToast('error', 'Navigation Failed', errorMessage);
  }
  ...
  selectDirectory(path, name) {
    ...
    if (this.elements.repoPath) {
      this.elements.repoPath.value = path;
    }

    // Auto-fill name if empty
    if (this.elements.repoName && !this.elements.repoName.value.trim()) {
      this.elements.repoName.value = name;
    }

    // Close browser and restore form
    this.restoreForm();

    // Update confirmation preview
    this.updateAddRepoPreview();

    this.showToast('success', 'Directory Selected', `Selected: ${name}`);
  }
  ...
  restoreForm() {
    ...
    if (modalBody) {
        ...
        <form id="addRepoForm">
          <div class="form-group">
            ...
            <input
              ...
              id="repoName"
              name="name"
              placeholder="e.g., waft"
              required
              autocomplete="off">
            <small class="form-hint">Display name for this repository</small>
          </div>
          <div class="form-group">
            ...
            <div class="path-input-group">
              <input
                ...
                id="repoPath"
                name="path"
                placeholder="/Users/ctavolazzi/Code/active/waft"
                required
                autocomplete="off">
              <button type="button" class="browse-btn" id="browsePathBtn">Browse...</button>
            </div>
            <small class="form-hint">Click "Browse..." to select a directory, or enter path manually</small>
          </div>
          <div class="add-repo-preview" id="addRepoPreview" style="display: none;">
            <div class="preview-header">
              <span class="preview-icon">✓</span>
              <span class="preview-title">Ready to Add</span>
            </div>
            <div class="preview-content">
              ...
              <div class="preview-item">
                ...
                <span class="preview-value" id="previewPath">-</span>
              </div>
            </div>
          </div>
        </form>
      ...
      this.elements.repoName?.addEventListener('input', () => this.updateAddRepoPreview());
      this.elements.repoPath?.addEventListener('input', () => this.updateAddRepoPreview());
  ...
  updateAddRepoPreview() {
    const preview = document.getElementById('addRepoPreview');
    const previewName = document.getElementById('previewName');
    const previewPath = document.getElementById('previewPath');
    const addRepoBtn = document.getElementById('addRepoConfirm');

    if (!preview || !previewName || !previewPath) return;
    ...
    if (name && path) {
      ...
      if (addRepoBtn) {
        addRepoBtn.classList.add('has-preview');
        addRepoBtn.textContent = `✓ Add "${name}"`;
      }
    } else {
      ...
      // Reset button
      if (addRepoBtn) {
        addRepoBtn.classList.remove('has-preview');
        addRepoBtn.textContent = 'Add Repository';
      }
    }
  }
  ...
  closeRepoForm() {
    ...
    // Reset form
    if (this.elements.addRepoForm) this.elements.addRepoForm.reset();
  }
  ...
  async submitAddRepo() {
    ...
    if (!name || !path) {
      this.showToast('error', 'Missing Fields', 'Please provide both name and path');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        this.showToast('error', 'Failed to Add Repository', data.error || 'Unknown error');
        return;
      }

      this.showToast('success', 'Repository Added', `Added "${name}" successfully`);
      this.closeRepoForm();
    } catch (error) {
      console.error('Add repo error:', error);
      this.showToast('error', 'Error', 'Failed to add repository');
    } finally {
      // Re-enable button
      if (this.elements.addRepoConfirm) {
        this.elements.addRepoConfirm.disabled = false;
        this.elements.addRepoConfirm.textContent = 'Add Repository';
      }
    }
  }
}
